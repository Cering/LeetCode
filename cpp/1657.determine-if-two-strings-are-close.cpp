/*
 * @lc app=leetcode id=1657 lang=cpp
 *
 * [1657] Determine if Two Strings Are Close
 *
 * https://leetcode.com/problems/determine-if-two-strings-are-close/description/
 *
 * algorithms
 * Medium (54.22%)
 * Likes:    4120
 * Dislikes: 359
 * Total Accepted:    561.1K
 * Total Submissions: 1M
 * Testcase Example:  '"abc"\n"bca"'
 *
 * Two strings are considered close if you can attain one from the other using
 * the following operations:
 * 
 * 
 * Operation 1: Swap any two existing characters.
 * 
 * 
 * For example, abcde -> aecdb
 * 
 * 
 * Operation 2: Transform every occurrence of one existing character into
 * another existing character, and do the same with the other
 * character.
 * 
 * For example, aacabb -> bbcbaa (all a's turn into b's, and all b's turn into
 * a's)
 * 
 * 
 * 
 * 
 * You can use the operations on either string as many times as necessary.
 * 
 * Given two strings, word1 and word2, return true if word1 and word2 are
 * close, and false otherwise.
 * 
 * 
 * Example 1:
 * 
 * 
 * Input: word1 = "abc", word2 = "bca"
 * Output: true
 * Explanation: You can attain word2 from word1 in 2 operations.
 * Apply Operation 1: "abc" -> "acb"
 * Apply Operation 1: "acb" -> "bca"
 * 
 * 
 * Example 2:
 * 
 * 
 * Input: word1 = "a", word2 = "aa"
 * Output: false
 * Explanation: It is impossible to attain word2 from word1, or vice versa, in
 * any number of operations.
 * 
 * 
 * Example 3:
 * 
 * 
 * Input: word1 = "cabbba", word2 = "abbccc"
 * Output: true
 * Explanation: You can attain word2 from word1 in 3 operations.
 * Apply Operation 1: "cabbba" -> "caabbb"
 * Apply Operation 2: "caabbb" -> "baaccc"
 * Apply Operation 2: "baaccc" -> "abbccc"
 * 
 * 
 * 
 * Constraints:
 * 
 * 
 * 1 <= word1.length, word2.length <= 10^5
 * word1 and word2 contain only lowercase English letters.
 * 
 * 
 */

// @lc code=start
class Solution {
public:
    bool closeStrings(string word1, string word2) {
        // 长度不同，不能互相转换
        if(word1.length() != word2.length())
            return false;

        std::unordered_map<char, int> umap1;
        for(const auto& ch : word1)
            umap1[ch]++;

        std::unordered_map<char, int> umap2;
        for(const auto& ch : word2) {
            umap2[ch]++;
        }

        // 两个字符串的字符集合不一致，不能互相转换
        std::unordered_map<int, int> cnt_umap;
        for(const auto& item : umap1) {
            if(umap2.find(item.first) == umap2.end())
                return false;
            cnt_umap[item.second]++;
        }
        for(const auto& item : umap2) {
            if(umap1.find(item.first) == umap1.end())
                return false;
            cnt_umap[item.second]--;
        }

        // 两个字符串的字符频次不一致，不能互相转换
        for(const auto& item : cnt_umap) {
            if(item.second != 0)
                return false;
        }

        return true;
    }
};
// @lc code=end

